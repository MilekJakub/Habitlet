...
export const CreateGoal = ({ onSuccess }: CreateGoalProps) => {
    ...
    if (isLastStep) {
      try {
          ...
          setValidationErrors(
            Object.entries(errors)
              ...
              .reduce((acc, [key, value]) => ({ 
                ...acc, 
                [key]: value && '_errors' in value && Array.isArray(value._errors) && value._errors.length > 0 
                  ? value._errors[0] 
                  : 'Invalid value' 
              }), {})
          );
        ...
        const when = formData.when
          ? format(formData.when, "yyyy-MM-dd")
          : "";
  ...
  const validateCurrentStep = (): boolean => {
    ...
    switch (step) {
      case 0: {
        const titleResult = createGoalInputSchema.shape.title.safeParse(formData.title);
      ...
      case 1: {
        const actionResult = createGoalInputSchema.shape.action.safeParse(formData.action);
      ...
      case 2: {
        const whatResult = createGoalInputSchema.shape.what.safeParse(formData.what);
      ...
      case 3: {
        const whenResult = createGoalInputSchema.shape.when.safeParse(formData.when);
      ...
      case 4: {
        const whereResult = createGoalInputSchema.shape.where.safeParse(formData.where);
      ...
      case 5: {
        const whichResult = createGoalInputSchema.shape.which.safeParse(formData.which);
      ...
      case 6: {
        const whyResult = createGoalInputSchema.shape.why.safeParse(formData.why);
      ...
      case 7: {
        const categoryResult = createGoalInputSchema.shape.category.safeParse(formData.category);
        
        if (!categoryResult.success) {
          setValidationErrors({ category: categoryResult.error.errors[0].message });
          return false;
        }
        ...
        const priorityResult = z.enum(["low", "medium", "high"] as const).safeParse(formData.priority);
  ...
  const handleNext = () => {
    if (validateCurrentStep()) {
      ...
      toast({
        title: "Validation Error",
        description: Object.values(validationErrors)[0] || "Please fill in all required fields.",
        variant: "destructive",
      });
    }
  };
  ...
  const renderStep = () => {
    ...
    switch (step) {
      case 0: return <TitleStep {...stepProps} />;
      case 1: return <ActionStep {...stepProps} />;
      case 2: return <WhatStep {...stepProps} />;
      case 3: return <WhenStep {...stepProps} />;
      case 4: return <WhereStep {...stepProps} />;
      case 5: return <WhichStep {...stepProps} />;
      case 6: return <WhyStep {...stepProps} />;
      case 7: return <DetailsStep {...stepProps} />;
      default: return null;
    }
  };
